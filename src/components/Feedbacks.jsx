...
const FeedbackCard = ({
  ...
  >
    ...
    <div className="mt-1">
      <p className="text-[18px] tracking-wider">{testimonial}</p>
      ...
      <div className="mt-7 flex justify-between items-center gap-1">
        ...
        <img src={image} alt="feedback" className="w-10 h-10 rounded-full object-contain" />
      </div>
    </div>
  </motion.div>
);
    ...
    <div className="mt-20 bg-black-100 rounded-[20px]">
      ...
      >
        <motion.div variants={textVariant()}>
          ...
          <h2 className={styles.sectionHeadText}>Мнение</h2>
        </motion.div>
      </div>

      <div className={`${styles.paddingX} -mt-20 pb-14 flex flex-wrap gap-7`}>
        {testimonials.map((tes, index) => (
          <FeedbackCard key={`${tes.name}_${index}`} index={index} {...tes} />
        ))}
      </div>
    </div>
  );
